...
const ExpenseForm: React.FC<ExpenseFormProps> = ({ onSubmit, initialData, onCancel }) => {
  ...
  const handleFormSubmit = (values: ExpenseFormValues) => {
    ...
    toast.success(initialData ? "Dépense modifiée avec succès !" : "Dépense ajoutée avec succès !");
  };
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input placeholder="Ex: Achat de fournitures" {...field} className="border-primary-erp focus:ring-primary-erp focus:border-primary-erp" />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
